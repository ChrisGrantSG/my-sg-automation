...
if ((ScenarioType == 'Donate_SignIn_Member') || (ScenarioType == 'Donate_SignIn_OrgAdmin')) {
	...
	************************************************/
    ...
    if (OrganizationCount != '1') {
        ...
        /***
		 *	double check to see if data says 'Donate_SignIn_OrgAdmin. 
		 *  If so, it clicks on member view drop down, if not continues without error
		 */
        if (ScenarioType == 'Donate_SignIn_OrgAdmin') {
            if (WebUI.verifyElementPresent(findTestObject('Object Repository/Page_Dashboard  New PCO Test Church/select_Admin ViewMember View'), 
                3, FailureHandling.OPTIONAL)) {
                WebUI.selectOptionByLabel(findTestObject('Object Repository/Page_Dashboard  New PCO Test Church/select_Admin ViewMember View'), 
                    'Member View', false)
            }
        } /***
		 *	New Code End
